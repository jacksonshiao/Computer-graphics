...
{
	FILE	*scene;
	char	token[100], buf[100], v[5][100];	// v[5] ��ܤ@�� polygon �̦h�i�H�� 5�� vertex
	float	vec[3];

	size_t	n_vertex, n_texture, n_normal;
	size_t	cur_mtl = 0;				// state variable: �ثe�ҨϥΪ� material
...
void mesh::LoadMtl(string tex_file)
{
	...
	while(!feof(fp_mtl))
	{
		...
		if (!strcmp(token,"newmtl"))
		{
			...
			cur_mat = mTotal++;					// �q mList[1] �}�l�AmList[0] �ŤU�ӵ� default material ��
